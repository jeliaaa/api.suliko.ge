"""Migration 0004 must build exactly what the portal and Drive models declare.

Same approach as ``test_migration_parity.py`` — the revision runs against a
recorder, no database — plus the check that matters most for this revision:
row-level security goes on the tenant tables and deliberately NOT on the
platform tables, and nothing is created that is not listed in either group.
"""
...
FILENAME = "0004_translator_portal_and_drive.py"
...
def test_unique_constraints_match_the_model(
    ...
    """The link table's uniqueness IS its access rule: one account per
    directory row, one row per bureau per account."""
...
def test_platform_tables_get_no_policy(revision: tuple[Any, RecordedOps]) -> None:
    """A tenant_isolation policy on the link table would make it unreadable
    before a tenant is bound — which is exactly when the portal reads it."""
    module, ops = revision
    sql = "\n".join(ops.statements)
